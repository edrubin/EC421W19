...
**Note:** These questions should help you review, along with the two problem sets. They are not necessarily comprehensive. I still suggest reviewing the problem sets and notes.
...
23\. Why do static time series models rarely make sense?

---
class: clear

24\. In time series models, why do we generally want to include lags of explanatory variables (and/or lags of the dependent variable)?

25\. Is OLS unbiased for static time-series models?

26\. Is OLS unbiased for time-series models with lagged explanatory variables?

27\. Is OLS unbiased for time-series models with lagged dependent (outcome) variables?

28\. For which time-series models is OLS consistent?

29\. Explain—simply and briefly—how we interpret $\beta_1$ in the model $\text{Births}_t = \beta_0 + \beta_1 \text{Births}_{t-1} + u_t$.

30\. How would you augment the static model $\text{Births}_t = \beta_0 + \beta_1 \text{Income}_{t} + u_t$ to account for income today, yesterday, and the previous day? In this new model, how would you estimate the *total effect* of income?

31\. What do we mean by *causality*?

32\. Why is causality important? Are there instances where correlation is also important/interesting?

33\. What do we mean by *prediction*? Does causality matter for prediction?

34\. Write down the model that each of the lines of .mono[R] code estimates. Would OLS be unbiased and/or consistent for each model? (Think about what could generate bias or inconsistency for each model—_i.e._, do any of the models violate our assumptions?) Think about the interpretation for the coefficients in each model.
  - `lm(y ~ x1 + x2)`
  - `lm(y ~ x1 + x2 + x1:x2)`
  - `lm(y ~ x1 + I(x1^2))`
  - `lm(log(y) ~ x1 + x1)`
  - `lm(log(y) ~ log(x1))`
  - `lm(y ~ x1 + lag(x1))`
  - `lm(y ~ x1 + lag(x1) + lag(x1, 2))`
  - `lm(y ~ x1 + lag(x1) + lag(y))`

```{R, generate pdfs, include = F, eval = T}
system("decktape remark midterm_review.html midterm_review.pdf --chrome-arg=--allow-file-access-from-files")
```
