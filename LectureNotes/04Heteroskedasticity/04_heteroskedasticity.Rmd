...
.footnote[
[†]: Goldfeld and Quandt suggested $n^{\star}$ of $(3/8)n$.
]
---
...
```{R, ex gq9}
# p-value
pf(q = f_gq, df1 = 158, df2 = 158, lower.tail = F)
```
---
...
*p*-value $\approx `r round(pf(q = f_gq, df1 = 158, df2 = 158, lower.tail = F), 5)`$
...
which has a *p*-value of approximately `r round(pf(sse_model4 / sse_model3, 158, 158, lower.tail = F), 4)`.
...
<br>.slate[H.sub[0]:] $\color{#314f4f}{\alpha_1 = \alpha_2 = \alpha_3 = 0}$ to
...
# Grab the R-squared
(white_r2 <- summary(bp_model)$r.squared)
```
---
count: false
...
**Step 4:** Calculate the associated *p*-value (where $\text{LM} \overset{d}{\sim} \chi_k^2$)
...
- **A:** It biases our standard errors—wrecking our statistical tests and confidence intervals.
---
count: false
